...
说白了就是帮你搞清楚 OpenClaw 各个 Agent 到底消耗了多少 Token。支持按 Agent、日期、时间范围各种维度分析，让你对 Token 消耗心里有数，好优化使用成本。

### 主要功能

- 每小时自动记录各 Agent 的 Token 用量快照
- 按 Agent、日期、Session 分组统计
- 支持 24 小时/7 天/30 天/周末/自定义范围查询
- 日志自动轮转（10MB 限制 + 90 天保留）
- macOS (launchd) 和 Linux (cron) 都能用
...
| 命令 | 说明 |
|------|------|
| `查 Token 用量` | 选个时间范围出报告 |
| `查上周末的 Token 用量` | 直接出上周末报告 |
| `查过去 7 天的用量` | 直接出 7 天报告 |
| `Token 用量分析` | 同上 |
...
**macOS (launchd)**:
编辑 `services/com.token-usage.collector.plist`，改 `StartCalendarInterval` 配置。
...
测试环境：macOS 14.x, OpenClaw 0.9.x, 2 个活跃 Agent
...
**读取**:
- `~/.openclaw/agents/*/sessions/*.jsonl` - 只读 usage 字段（Token 统计）
...
可能原因：
1. 刚安装，数据还在积累（建议 24 小时后查）
2. 定时任务没正常运行（跑 `collect-usage --diagnose` 检查）
...
自动轮转已经启用，也可以手动跑 `collect-usage --cleanup` 清理。
...
- 支持所有模型的 Token 估算（不仅限 Bailian）
- 有真实数据用真实数据，没数据自动估算
- 支持 OpenAI/Anthropic/Bailian 等多种模型定价
- OpenAI/Codex 统计结果跟 Dashboard 100% 一致
- Bailian 等模型估算准确度 90-95%
- 修复：正确提取 API 返回的 cost 数据（usage.cost.total）
- 添加 OpenClaw Issue 草稿，提交官方修复请求

### v1.1.0 (2026-03-13)

- collector.py 改成增量统计，避免重复累加历史数据
- 新增状态文件 `collector-state.json` 记录每个 session 的累计进度
- analyzer.py 移除 cost 显示（因为模型配置中 cost 都是 0）
- 数据准确性跟 OpenClaw Dashboard 保持一致
...
1. 跑诊断：`collect-usage --diagnose`
2. 查日志：`~/.openclaw/logs/token-usage-collector.log`
3. 提 Issue：[GitHub Issues](https://github.com/jackwude/token-usage-analysis/issues)
