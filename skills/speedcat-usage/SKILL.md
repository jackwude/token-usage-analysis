...
### Profile 持久化（优先使用）
优先使用独立 Chrome Profile：
- Profile 目录：`~/.openclaw/chrome-profiles/speedcat/`
- 启动脚本：`~/.openclaw/scripts/start-speedcat-chrome.sh`
- 调试端口：`9223`

### Cookies 注入（仅作兜底）
保存的 cookies 位于 `~/.openclaw/cookies/speedcat.json`，仅在 Profile 尚未初始化或临时应急时使用。
...
### 🚀 推荐模式（优先）：Profile 持久化
1. **检查/启动专用 Chrome**：运行 `~/.openclaw/scripts/start-speedcat-chrome.sh`
2. **确认使用专用 Profile**：必须是 `~/.openclaw/chrome-profiles/speedcat/`
3. **访问用户页**：打开 `https://speedcat.co/user`
4. **检查是否已登录**：若页面已进入用户中心，则直接继续
5. **如未登录**：进入完整登录流程（见下方）
6. **登录成功后继续复用此 Profile**：不要把 cookies 作为主保存方式

### 📝 完整登录流程（Profile 掉登录时）
1. **打开专用 Profile 浏览器**：保持 `speedcat` Profile 不变
2. **通过验证**：点击"我不是恶意刷站"按钮（访问验证页面）
3. **登录**：
   - 优先从 macOS Keychain 读取账号密码
   - 若 Keychain 不可用，再从 `TOOLS.md` 读取 SpeedCat 账号（邮箱 + 密码）
   - 填写邮箱和密码，点击登录按钮
4. **登录成功后保留 Profile**：后续任务继续复用同一 Profile
5. **可选更新 Cookies**：仅作为应急备份，成功登录后可以顺手更新 `~/.openclaw/cookies/speedcat.json`

### 🪂 兜底模式：Cookies 注入
1. **检查 cookies 文件**：读取 `~/.openclaw/cookies/speedcat.json`
2. **打开浏览器**：访问 `https://speedcat.co/user`
3. **注入 Cookies**：使用 `document.cookie` 注入保存的登录态
4. **刷新页面**：验证是否已登录
5. **如未登录**：立即回退到 Profile + 完整登录流程
...
1. **优先使用 Profile 持久化**：固定使用 `~/.openclaw/chrome-profiles/speedcat/`
2. **Cookies 只作兜底**：注入后若未显示登录态（如用户名、钱包余额），立即回退到 Profile 登录流程
